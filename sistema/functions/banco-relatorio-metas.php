<?php
	class bancorelatoriometas extends banco{
	    
	    #pdf relatório
	    function MontaImpressao($dataIni, $dataFim, $idresponsavel, $marca, $idtipopagamento, $cidade){
	        #Inicia mPDF
	        ...
	        $mpdf = new mPDF('utf-8', 'A4-L', '', 'Helvetica', 8, 8, 9, 9, 0, 3);
	        $mpdf->SetFooter('{PAGENO} / {nb}');
	        #HTML Auxilio
	        $Auxilio = utf8_encode(parent::CarregaHtml('impressao-relatorio-venda'));
	        ...
	        $Auxilio = str_replace('<%DATAINI%>', date("d/m/Y", strtotime($dataIni)), $Auxilio);
	        $Auxilio = str_replace('<%DATAFIM%>', date("d/m/Y", strtotime($dataFim)), $Auxilio);
	        $Auxilio = str_replace('<%FUNCIONARIO%>', parent::BuscaUsuarioPorId($idresponsavel), $Auxilio);
	        
	        if($idtipopagamento){
	            $SqlPagamento = "SELECT forma_pagamento FROM fixo_forma_pagamento WHERE idformapagamento = " . $idtipopagamento;
	            $resultPagamento = parent::Execute($SqlPagamento);
	            $rsPagamento = parent::ArrayData($resultPagamento);
	            $pagamento = "/ Forma Pagamento: " . $rsPagamento['forma_pagamento'];
	        }
	        
	        if($marca){
	            $str_marca = "/ Marca: " . $marca;
	        }
	        if($cidade){
	            $str_cidade = " / Cidade: " . $cidade;
	        }
	        $Auxilio = str_replace('<%TIPOPAGAMENTO%>', $pagamento, $Auxilio);
	        $Auxilio = str_replace('<%MARCA%>', $str_marca, $Auxilio);	        
	        $Auxilio = str_replace('<%CIDADE%>', $str_cidade, $Auxilio);
	        
	        #SQL busca
	        $Aux_Linha = parent::CarregaHtml('itens/impressao-relatorio-venda-itens');
	        $where = '';
	        if($dataIni){
	        ...
	        $LinhaTotal = parent::CarregaHtml('itens/impressao-relatorio-venda-ultima');
	        ...
	        $Auxilio = str_replace('<%VENDAS%>', $Relatorio, $Auxilio);
		...
		function MontaRelatorio($dataIni, $dataFim, $idresponsavel, $marca, $idtipopagamento, $cidade){
			$Auxilio = parent::CarregaHtml('itens/relatorio-metas-itens');
			$where = '';
			if($dataIni){
				#$newdataIni = implode("-", array_reverse(explode("/", $dataIni)));
				$where .= " AND V.data >= '$dataIni 00:00:00'";
			...
			if($marca){
				...
				$Sql = "SELECT DISTINCT C.nome, V.idvenda, V.data, V.valor_frete, V.valor_venda, V.idusuario, V.desconto_subtotal FROM t_vendas V 
						INNER JOIN t_clientes C ON V.idcliente = C.idcliente
						LEFT JOIN t_vendas_pagamentos Y ON Y.idvenda = V.idvenda
						WHERE 1 $where AND V.orcamento = 0";
			}
			...
			while($rs = parent::ArrayData($result)){
				...
				if($marca){
				    ...
				    $SqlValorRelatorio = "SELECT SUM((valor_relatorio * quantidade)-desconto_valor) AS valor_relatorio FROM t_vendas_produtos WHERE idvenda = " . $rs['idvenda'] . " AND brinde = 0";
				    $resultValorRelatorio = parent::Execute($SqlValorRelatorio);
				    $rsValorRelatorio = parent::ArrayData($resultValorRelatorio);
				    $venda_sem_frete = $rsValorRelatorio['valor_relatorio'] - $rs['desconto_subtotal'];
